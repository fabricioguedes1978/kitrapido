...
SET pickup_info = NULLIF(trim(both ' · ' FROM concat_ws(' · ',
      NULLIF(pickup_days,''),
      CASE WHEN pickup_start_time IS NOT NULL AND pickup_end_time IS NOT NULL
           THEN to_char(pickup_start_time,'HH24:MI') || ' às ' || to_char(pickup_end_time,'HH24:MI')
           WHEN pickup_start_time IS NOT NULL THEN to_char(pickup_start_time,'HH24:MI')
           ELSE NULL END)), '')
WHERE pickup_info IS NULL;
