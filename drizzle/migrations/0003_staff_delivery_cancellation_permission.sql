...
ADD COLUMN can_cancel_deliveries boolean NOT NULL DEFAULT false;

ALTER TABLE public.team_invites
ADD COLUMN can_cancel_deliveries boolean NOT NULL DEFAULT false;

CREATE OR REPLACE FUNCTION public.can_cancel_delivery(_event_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
  ...
  SELECT public.is_admin()
    OR EXISTS (
      ...
      WHERE em.event_id = _event_id
        ...
        AND (
          em.role = 'organizer'::public.app_role
          OR (em.role = 'attendant'::public.app_role AND em.can_cancel_deliveries)
        )
    )
$$;

REVOKE ALL ON FUNCTION public.can_cancel_delivery(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.can_cancel_delivery(uuid) TO authenticated;

CREATE OR REPLACE FUNCTION public.guard_delivery_cancellation()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  IF NEW.status = 'cancelled'::public.delivery_status
     AND OLD.status IS DISTINCT FROM NEW.status
     AND NOT public.can_cancel_delivery(OLD.event_id) THEN
    RAISE EXCEPTION 'Você não tem autorização para cancelar entregas neste evento.';
  END IF;
  RETURN NEW;
END;
$$;

REVOKE ALL ON FUNCTION public.guard_delivery_cancellation() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.guard_delivery_cancellation() TO service_role;

CREATE TRIGGER deliveries_guard_cancellation
BEFORE UPDATE ON public.deliveries
FOR EACH ROW EXECUTE FUNCTION public.guard_delivery_cancellation();

CREATE OR REPLACE FUNCTION public.create_team_invite_with_permissions(
  _event_id uuid,
  _cpf text,
  _name text,
...
BEGIN
  ...
  IF existing_user_id IS NOT NULL THEN
    ...
    IF NOT EXISTS (
      ...
      IF EXISTS (
        SELECT 1 FROM public.event_members em
        WHERE em.user_id = existing_user_id
      ) THEN
    ...
    INSERT INTO public.event_members (event_id, user_id, role, can_cancel_deliveries)
    VALUES (_event_id, existing_user_id, _role, allowed_to_cancel)
  ...
  INSERT INTO public.team_invites (
    event_id, cpf, name, role, code_hash, status, invited_by, expires_at, can_cancel_deliveries
  ) VALUES (
    _event_id,
    normalized_cpf,
    clean_name,
...
REVOKE ALL ON FUNCTION public.create_team_invite_with_permissions(uuid, text, text, public.app_role, boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.create_team_invite_with_permissions(uuid, text, text, public.app_role, boolean) TO authenticated;

CREATE OR REPLACE FUNCTION public.list_team_invites_with_permissions(_event_id uuid)
RETURNS TABLE(
  id uuid,
  cpf text,
  name text,
...
BEGIN
  ...
  END IF;
  RETURN QUERY
  SELECT ti.id, ti.cpf, ti.name, ti.role, ti.status, ti.created_at, ti.expires_at, ti.can_cancel_deliveries
...
REVOKE ALL ON FUNCTION public.list_team_invites_with_permissions(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.list_team_invites_with_permissions(uuid) TO authenticated;

CREATE OR REPLACE FUNCTION public.set_attendant_cancel_permission(_member_id uuid, _allowed boolean)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  SELECT event_id, role INTO target_event_id, target_role
  FROM public.event_members
  WHERE id = _member_id;

  IF target_event_id IS NULL OR NOT public.can_manage_event(target_event_id) THEN
    RAISE EXCEPTION 'Você não tem permissão para alterar este staff.';
  END IF;
  IF target_role <> 'attendant'::public.app_role THEN
    RAISE EXCEPTION 'Esta permissão só pode ser alterada para staff.';
  END IF;

  UPDATE public.event_members
  SET can_cancel_deliveries = COALESCE(_allowed, false)
  WHERE id = _member_id;
  RETURN true;
END;
$$;

REVOKE ALL ON FUNCTION public.set_attendant_cancel_permission(uuid, boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.set_attendant_cancel_permission(uuid, boolean) TO authenticated;
...
BEGIN
  ...
  IF NEW.email LIKE '%@equipe.cronochip.app' THEN
    ...
    INSERT INTO public.event_members (event_id, user_id, role, can_cancel_deliveries)
    VALUES (claimed_invite.event_id, NEW.id, claimed_invite.role, claimed_invite.can_cancel_deliveries)
    ON CONFLICT (event_id, user_id) DO UPDATE
      SET role = EXCLUDED.role,
          can_cancel_deliveries = EXCLUDED.can_cancel_deliveries;
